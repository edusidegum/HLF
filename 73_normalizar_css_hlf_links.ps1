...
$File = "D:\GitHub\HLF\links.html"; $TargetCssHref = "assets/css/links.css"
if (-not (Test-Path $File)) { throw "Arquivo nao encontrado: $File" }
$html = Get-Content $File -Raw -Encoding UTF8
$pattern = '<link[^>]+rel=["'']stylesheet["''][^>]+href=["'']([^"'']*(?:style|styles|links)\.css)["''][^>]*>'
...
Set-Content -Path $File -Value $html -Encoding UTF8; Write-Host "CSS links normalizado para: $TargetCssHref"
