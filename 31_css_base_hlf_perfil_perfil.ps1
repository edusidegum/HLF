...
$CssFile = 'D:\GitHub\HLF\perfil\assets\css\perfil.css'
...
header + section {
  background:
    radial-gradient(circle at top right, rgba(31,111,74,.06), transparent 28%),
...
input[type="submit"]:hover {
  ...
  box-shadow: 0 10px 24px rgba(21,77,52,.16);
  text-decoration: none;
}
...
.kicker {
  ...
  background: var(--accent-soft);
  color: var(--accent-2);
  border: 1px solid rgba(21,77,52,.10);
  font-size: .82rem;
  font-weight: 700;
  letter-spacing: .04em;
...
aside,
blockquote {
  animation: fadeUpSoft var(--anim-base) ease both;
}
...
h2::after {
  ...
  animation: lineGrow var(--anim-slow) ease both;
}
